...
func (s *State) PromptWithSuggestionReverse(prompt string, text string, pos int) (string, error) {
	return s.doPrompt(prompt, text, pos, true)
}

func (s *State) doPrompt(prompt string, text string, pos int, reverseSearch bool) (string, error) {
	...
	s.historyProvider.RLock()
	defer s.historyProvider.RUnlock()
